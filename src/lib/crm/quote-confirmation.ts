...
export function quoteReadyToBookSms(opts: {
  ...
  if (opts.lang === "es") {
    return lines(
      `Hola ${name} — Toro Movers`,
      ``,
      `Si el precio te funciona, elige tu horario.`,
      `El depósito de ${DEPOSIT_AMOUNT_DISPLAY} se cobra al reservar y se descuenta de la factura final.`,
      ``,
      `Agenda aquí:`,
      DEPOSIT_LINK,
      ``,
  ...
  return lines(
    `Hi ${name} — Toro Movers`,
    ``,
    `If the quote works, pick your time.`,
    `The ${DEPOSIT_AMOUNT_DISPLAY} deposit is taken at booking and comes off your final invoice.`,
    ``,
    `Book here:`,
    DEPOSIT_LINK,
    ``,
